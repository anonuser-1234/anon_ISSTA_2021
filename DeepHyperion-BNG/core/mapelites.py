...
class MapElites(ABC):
    ...
    def run(self):
        ...
        for i in range(0, self.iterations):
            if Config.EXECTIME <= self.config.RUNTIME:
                ...
                if Config.EXECTIME >= self.config.INTERVAL * ii:
                    self.extract_results(i, (self.config.INTERVAL * ii), self.solutions, self.performances)
                    ii += 1
            else:
                break
    ...
    def extract_results(self, iterations, execution_time, solutions, performances, rescaled=False):
        ...
        for (i, j), value in np.ndenumerate(performances):
            ...
            if performances[i, j] != np.inf:
                ...
                # Path(destination_sim).mkdir(parents=True, exist_ok=True)
                destination_road = f"{log_dir_path}\\road_{solutions[i,j].m.name}_{i,j}.json"
                # Path(destination_road).mkdir(parents=True, exist_ok=True)
        ...
        report = {
            ...
            'Misbehaviour density': str(Individual.COUNT_MISS / filled),
        ...
        self.plot_map_of_elites(performances, f"logs/{log_dir_name}")
